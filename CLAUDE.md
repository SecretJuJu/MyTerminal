...
- **Tab 완성은 앱이 만든다**(`CommandCompletion`). 셸에 물어보는 길이 없기 때문이다 — 상자의 글은 셸의 줄 버퍼에 없어서 Tab을 넘겨도 완성할 것이 없고, 결과를 화면에서 긁는 방법은 프롬프트 모양에 기댄다. 그래서 경로와 `PATH` 실행 파일만 찾는다. 디렉터리 조회와 실행 파일 목록은 `Sources`로 주입해 디스크 없이 검증한다.
- **셸 히스토리는 파일에서 직접 읽는다**(`ShellHistory`). zsh는 `: <시각>:<걸린 시간>;<명령>` 형식에 줄 끝 역슬래시로 여러 줄을 잇고, 아스키가 아닌 바이트를 `0x83` + (바이트 xor 32)로 적는다 — 이걸 풀지 않으면 한글 명령이 깨진다.
- 읽은 셸 히스토리는 `CommandHistory.seed`로 깔기만 하고 저장하지 않는다. 사용자의 히스토리를 우리 파일에 복제하면 토큰 같은 것이 한 벌 더 생긴다.
- 되부르기 중 사용자가 글을 고치면 `endRecall()`을 불러야 한다. 안 그러면 다음 ↑가 새로 친 글이 아니라 아까 훑던 자리에서 이어진다(접두사 검색이 통째로 무시됐다).
